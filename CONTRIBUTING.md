...
1. **Base game** — a hardcoded table in `src/Plugin.cs` maps the 11 vanilla `PunManager.UpgradePlayer*(string, int)` methods to their corresponding `Dictionary<string, int>` field on `StatsManager`. The mapping is hardcoded because the method/dict names don't follow a derivable rule (e.g. `UpgradePlayerSprintSpeed` writes to `playerUpgradeSpeed`, `UpgradePlayerEnergy` writes to `playerUpgradeStamina`).
2. **Modded** — every loaded assembly is scanned for methods matching the same shape (`int UpgradePlayer*(string, int)`). For each match, the method body's IL is scanned for the first `ldfld`/`stfld` referencing a `Dictionary<string, int>` field on `StatsManager`. That field becomes the count source for the cap.

If you're adding a modded upgrade and want UpgradeLimiter to pick it up automatically, follow the base-game pattern: name the method `UpgradePlayer<Something>`, give it the `(string steamID, int value = 1)` signature, and have it touch a `Dictionary<string, int>` field on `StatsManager` before doing anything else.
